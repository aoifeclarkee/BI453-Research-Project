---
title: "BI453- Brain Cancer Gender"
author: "AC"
date: "2025-01-25"
...
```{r}
#Check dimensions of matrix formation
dim(exprs)
```
...
```{r}
# Check the expression matrix and metadata for gender
head(exprs)
head(Corresdata$gender)
```
...
```{r}
# Check for samples with missing gender information 
...
design <- model.matrix(~ factor(aligned_metadata$gender))
...
# View the top differentailly expressed genes (DEGs)
topTable(fit, coef = "Male_vs_Female", adjust.method = "BH", p.value = 0.05)
```


```{r}
# Identify DEGs based on p-value threshold
degs <- topTable(fit, coef = "Male_vs_Female", adjust.method = "BH", p.value = 0.05, number = Inf)

# Get the amount of the significant DEGs to be removed
...
exprs_filtered <- exprs[!rownames(exprs) %in% degs_genes, ]

```


```{r}
# Analyse the matrix before and after gene removal to ensure the DEGs between male and female samples have been removed correctly
dim(exprs)
dim(exprs_filtered)
```
...
for (x in 1:num_runs) {
  # Run NMF on the original matrix (exprs)
...
for (x in 1:num_runs) {
  # Run NMF on the filtered matrix (exprs_filtered)
  nmf_result_filtered <- nmf(exprs_filtered, rank = 2, nrun = 2, seed = "random")
  nmf_results_filtered[[as.character(x)]] <- nmf_result_filtered
}

# first run on the original matrix
summary(nmf_results_original[[1]])

# first run on the filtered matrix
...
# Generate cluster predictions for each NMF run
for (x in 1:100) {
  # Extract NMF results
...
```{r}
#After correct alignment, create predictions
for (x in 1:100) {
...
```{r}
#Purity
#Make lists to store purity values
...
for (x in 1:100) {
  if (!is.null(predictions_original[[x]]) && length(predictions_original[[x]]) == length(groundtruths)) {
    ...
    cat("Skipping run", x, "- Missing predictions or mismatched lengths\n")
  }
...
```{r}
# Find valid indices (no NA values in groundtruths)
...
```{r}
# Check length of groundtruths
length(groundtruths)

# Check length of the first set of predictions (original and filtered)
length(predictions_original[[1]])
length(predictions_filtered[[1]])
...
# Complete the same for the filtered matrix
...
# length of the first set of predicted clusters from the original matrix
length(H2_clusters_original[[1]])

# length of the first set of predicted clusters from the filtered matrix
length(H2_clusters_filtered[[1]])
```


```{r}
# Purity Post-Processing
# Inputs are all Post-Processing
...
capped_p_value_log <- min(p_value_log, 20)
...
ari_p_value_log_capped <- min(ari_p_value_log, 20)

# Create a data frame for plotting results
...
ggplot(purity_comparison, aes(x = Condition, y = Average_Purity, fill = Condition)) +
  ...
  scale_fill_manual(values = c("Pre-Processed" = "lightblue", "Post-Processed" = "navy")) +
  labs(
...
```{r}
#Clustering Errors vs. Number of genes
...
selected_genes_filtered <- head(order(apply(exprs_filtered, 1, var), decreasing = TRUE), num_genes)
exprs_filtered_subset <- exprs_filtered[selected_genes_filtered, ]
