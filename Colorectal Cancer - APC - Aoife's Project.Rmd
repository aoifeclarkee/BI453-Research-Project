---
title: "Colorectal Cancer 2 - Aoife Project"
author: "Aoife Clarke"
date: "2025-02-12"
...
# If either is not a data frame, convert them
...
```{r}
# Filter Out Differentially Expressed Genes (DEGs) Based on APC mutation
...
#(This does not need to be done but I was interested to see)

# Perform PCA on the original and filtered expression matrices
pca_original <- PCA(t(exprs), graph = FALSE)
pca_filtered <- PCA(t(filtered_exprs_apc), graph = FALSE)
...
#Find the Optimal Number of Genes for Clustering by choosing a different gene threshold

# Define a range of gene counts to test
gene_thresholds <- seq(600, 950, by = 50)
...
```{r}
# Define a range of gene counts to test
gene_thresholds <- seq(700, 950, by = 50)
...
```{r}
# Define a range of gene counts to test
gene_thresholds <- seq(800, 950, by = 50)
