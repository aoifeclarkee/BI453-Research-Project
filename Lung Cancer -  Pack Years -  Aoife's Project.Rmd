---
title: "BI453 Lung Cancer - Aoife's Project"
author: "Aoife Clarke"
date: "2025-02-19"
...
```{r}
print(table(sample_metadata$`packyears:ch1`))
```

```{r}

sample_metadata$`packyears:ch1` <- as.numeric(sample_metadata$`packyears:ch1`)

# Check the median pack-years 
packyears_median <- median(sample_metadata$`packyears:ch1`, na.rm = TRUE)

# Create a high vs. low pack-years
sample_metadata$pack_years_group <- ifelse(sample_metadata$`packyears:ch1` > packyears_median, "High", "Low")

# Check how many samples are in each group
table(sample_metadata$pack_years_group)
...
# Ensure the sample order in exprs matches sample_metadata
...
design <- model.matrix(~ factor(sample_metadata$pack_years_group))
...
deg_results <- topTable(fit, coef = "High_vs_Low", number = Inf, adjust.method = "fdr")
...
filtered_exprs_lung <- exprs[!(rownames(exprs) %in% genes_to_remove), ]

# Check new dimensions after filtering
dim(filtered_exprs_lung)
```
```{r}
...
for (x in 1:num_runs) {
  ...
  nmf_result_filtered <- nmf(filtered_exprs_lung, rank = rank, nrun = 1, seed = "random")
...
for (x in 1:num_runs) {
  ...
  names(cluster_predictions_filtered) <- colnames(filtered_exprs_lung)
...
# Ensure order matches groundtruths
...
```{r}
# Paired Two-Sided t-test and SEM for Normalized Mutual Information
...
gene_counts <- seq(500, 967, by = 100)  
...
for (num_genes in gene_counts) {
  ...
  selected_genes_filtered <- head(order(apply(filtered_exprs_lung, 1, var), decreasing = TRUE), num_genes)
  exprs_filtered_subset <- filtered_exprs_lung[selected_genes_filtered, ]
